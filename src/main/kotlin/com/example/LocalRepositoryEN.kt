...
class LocalRepositoryEN(private val dataSource: DataSource) {
    private fun insertLog(name: String, size: Int = 0) {
        logger.info("Inserted $name [EN] to Local Server")
    }
    ...
    fun insertModuleMetaData(modules: List<ModuleOriginalData>){
        dataSource.connection.use { conn ->
            ...
            conn.prepareStatement(
                """
                    INSERT OR REPLACE INTO ModuleEntity_EN VALUES (?, ?, ?, ?, ?, ?, ?)
                """.trimIndent()
            ).use { stmt ->
...
//              insertLog("Modules", modules.size)
            ...
            conn.prepareStatement(
                """
                    INSERT OR REPLACE INTO ModuleStatEntity_EN VALUES (?, ?, ?, ?)
                """.trimIndent()
            ).use { stmt ->
...
//                insertLog("ModuleStat", modules.size)
    ...
    fun insertDescendantsMetadata(descendants: List<DescendantOriginalData>) {
        ...
        dataSource.connection.use { conn ->
            ...
            conn.prepareStatement(
                """
                    INSERT OR REPLACE INTO Descendants_EN VALUES (?, ?, ?)
                """.trimIndent()
            ).use { stmt ->
...
//////////                insertLog("Descendants", descendants.size)
            ...
            conn.prepareStatement(
                """
                    INSERT OR REPLACE INTO DescendantSkills_EN VALUES (?, ?, ?, ?, ?, ?, ?)
                """.trimIndent()
            ).use { stmt ->
...
////////                insertLog("Descendants Skill Entity (List Size) ", descendants.sumOf { it.descendant_skill.size })
            ...
            conn.prepareStatement(
                """
                    INSERT OR REPLACE INTO DescendantStatDetail_EN VALUES (?, ?, ?, ?)
                """.trimIndent()
            ).use { stmt ->
                descendants.flatMap { entity ->
...
//////                insertLog("Descendants Stat Detail Entity (List Size) ", descendants.sumOf { it.descendant_stat.size})
    ...
    fun insertTitleMetadata(titles: List<TitleOriginalData>) {
        dataSource.connection.use { conn ->
            ...
            conn.prepareStatement(
                """
                    INSERT OR REPLACE INTO TitleEntity_EN VALUES (?, ?)
                """.trimIndent()
            ).use {stmt ->
...
////                insertLog("TitleEntity", titles.size)
    ...
    fun insertStatMetaData(stats: List<StatOriginalData>){
        dataSource.connection.use { conn ->
            ...
            conn.prepareStatement(
                """
                    INSERT OR REPLACE INTO StatEntity_EN VALUES (?, ?)
                """.trimIndent()
            ).use { stmt ->
                stats.forEach {stat ->
...
//                insertLog("Stat Entity", stats.size)
    ...
    fun insertWeaponMetadata(weapons: List<WeaponOriginalData>){
        dataSource.connection.use { conn ->
            ...
            conn.prepareStatement(
                """
                    INSERT OR REPLACE INTO WeaponEntity_EN VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """.trimIndent()
            ).use { stmt ->
...
//                insertLog("Weapon Entity", weapons.size)
            ...
            conn.prepareStatement(
                """
                    INSERT OR REPLACE INTO WeaponBaseStatEntity_EN VALUES (?, ?, ?)
                """.trimIndent()
            ).use { stmt ->
...
//                insertLog("Weapon BaseStat Entity", weapons.sumOf { it.base_stat.size })
            ...
            chunkValues.chunked(10000).forEach { chunk ->
                conn.prepareStatement(
                    """
                    INSERT OR REPLACE INTO WeaponFirearmEntity_EN VALUES (?, ?, ?, ?)
                """.trimIndent()
                ).use { stmt ->
...
//                    insertLog("Weapon Firearm Entity (Chunk)", chunk.size)
    ...
    fun insertReactorMetaData(reactors: List<ReactorOriginalData>){
        dataSource.connection.use { conn ->
            ...
            conn.prepareStatement("""
                INSERT OR REPLACE INTO ReactorEntity_EN VALUES (?, ?, ?, ?, ?)
            """.trimIndent()
            ).use { stmt ->
...
//                insertLog("Reactor Entity", reactors.size)
            ...
            conn.prepareStatement("""
                INSERT OR REPLACE INTO ReactorSkillPowerEntity_EN VALUES (?, ?, ?, ?)
            """.trimIndent()
            ).use { stmt ->
...
//                insertLog("ReactorSkillPower Entity")
            ...
            conn.prepareStatement("""
                INSERT OR REPLACE INTO ReactorEnchantEffectEntity_EN VALUES (?, ?, ?, ?, ?)
            """.trimIndent()
            ).use { stmt ->
                reactors.flatMap { reactor ->
...
//                insertLog("ReactorEnchantEffect Entity")
            ...
            conn.prepareStatement("""
                INSERT OR REPLACE INTO ReactorSkillPowerCoefficientEntity_EN VALUES (?, ?, ?, ?)
            """.trimIndent()
            ).use { stmt ->
                reactors.flatMap { reactor ->
                    reactor.reactorSkillPower.flatMap { reactorSp ->
...
//                insertLog("ReactorSkillPowerCoefficient Entity")
    ...
    fun insertExternalCompMetaData(exlist: List<ExternalComponentOriginalData>){
        dataSource.connection.use { conn ->
            ...
            conn.prepareStatement("""
                INSERT OR REPLACE INTO ExternalCompEntity_EN VALUES (?, ?, ?, ?, ?)
            """.trimIndent()
            ).use { stmt ->
                exlist.forEach { ex ->
...
//                insertLog("ExternalComp Entity")
            ...
            conn.prepareStatement("""
                INSERT OR REPLACE INTO ExternalCompBaseStatEntity_EN VALUES (?, ?, ?, ?)
            """.trimIndent()
            ).use { stmt ->
                exlist.flatMap { ex ->
...
//                insertLog("ExternalComp BaseStat Entity")
            ...
            conn.prepareStatement("""
                INSERT OR REPLACE INTO ExternalCompSetOptionEntity_EN VALUES (?, ?, ?, ?)
            """.trimIndent()
            ).use { stmt ->
                exlist.flatMap { ex ->
...
//                insertLog("ExternalComp SetOption Entity")
    ...
    fun insertTierMetaDataEN(list: List<Tier>) {
        dataSource.connection.use {conn ->
            conn.autoCommit = false

            conn.prepareStatement("""
                INSERT OR REPLACE INTO TierEntity_EN (tier_id, tier_name) VALUES (?, ?)
            """.trimIndent()
            ).use { stmt ->
                list.forEach { tier ->
